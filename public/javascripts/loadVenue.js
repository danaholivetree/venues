$(document).ready(function() {
  ...
  const showVenue = data => {
    ...
    $('.info.thumb-up span').text(`${up}`)
    $('.info.thumb-down span').text(`${down}`)
    ...
    if (vote === 'up') {
      $(`.upvote button`).css("color", "green")
    }
    if (vote === 'down') {
      $(`.downvote button`).css("color", "red")
    }
    setThumbListener()
  ...
  const setThumbListener = () => {
    $('.thumb-up').click( e => {
      console.log('clicked thumb up');
        $.post(`/api/votes`, {venueId: e.target.dataset.id, vote: 'up'}, data => {
          $(`.thumb-up span`).text(`${data.up}`)
          $(`.thumb-up button`).css("color", "green")
          $(`.thumb-down span`).text(`${data.down}`)
          $(`.thumb-down button`).css("color", "black")
        })
    })
    ...
    $('.thumb-down').click( e => {
        $.post(`/api/votes`, {venueId: e.target.dataset.id, vote: 'down'}, data => {
          $(`.thumb-up span`).text(`${data.up}`)
          $(`.thumb-up button`).css("color", "black")
          $(`.thumb-down span`).text(`${data.down}`)
          $(`.thumb-down button`).css("color", "red")
        })
    })
  }
